...
class EnrichedTextInputView : AppCompatEditText {
  ...
  private fun toggleStyle(name: String) {
    when (name) {
      ...
      EnrichedSpans.CHECK_ITEM -> paragraphStyles?.toggleStyle(EnrichedSpans.CHECK_ITEM)
  ...
  private fun removeStyle(name: String, start: Int, end: Int): Boolean {
    val removed = when (name) {
      ...
      EnrichedSpans.CHECK_ITEM -> paragraphStyles?.removeStyle(EnrichedSpans.H3, start, end)
  ...
  private fun getTargetRange(name: String): Pair<Int, Int> {
    val result = when (name) {
      ...
      EnrichedSpans.CHECK_ITEM -> paragraphStyles?.getStyleRange()
